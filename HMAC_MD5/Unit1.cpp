...
void my32_hmac_md5(char *str, int str_len, unsigned char *key, int key_len, char rtn[33]);
...
void my32_hmac_md5(char *str, int str_len, unsigned char *key, int key_len, char rtn[33]){
	...
	//將轉出來的字串裡面英文字母小寫都轉成大寫        
	for(i = 0; i < 33; i ++){
		if(rtn[i] <= 0x7A && rtn[i] >= 0x61)  // 0x7A='z'  0x61='a'
			rtn[i] -= 0x20;
	}
}
//---------------------------------------------------------------------------
...
{
        strcpy( mac_str , Edit1->Text.c_str() );
        //strcpy( key , Edit2->Text.c_str() );
        strcpy( key , "1234" );
        my32_hmac_md5((char *)mac_str, strlen(mac_str), key, strlen(key), (char*)secret_key);
        Edit2->Text = secret_key;
}
//---------------------------------------------------------------------------

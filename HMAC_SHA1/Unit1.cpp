...
void __fastcall TForm1::showKey(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
        AnsiString six_byte_mac = Edit1->Text.SubString( 7 , 6 ); ;
        strcpy( mac_str , six_byte_mac.c_str() );
        mac_str[7] = 0;
        strcpy( key , Edit2->Text.c_str() );
        sha1_hmac(key, strlen(key), mac_str, strlen(mac_str), (char*)secret_key);
        char secret_key_hex[41];
        BinToHex( secret_key, secret_key_hex, 20 );
        secret_key_hex[40] = 0;
        Edit3->Text = secret_key_hex;
}
//---------------------------------------------------------------------------
